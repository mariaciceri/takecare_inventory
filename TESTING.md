...
| After clicking edit, if user went back to the orders page and deleted the order, its content remained available for editing. | Clear the session after deleting the order and verify if it's empty to remove localStorage data. |
| Alert messages for stock levels, users approval and expiration date were displaying in the admin login page. | Move the message block from `base.html` to `index.html`, which initially causes the bug described below. Then, implement a session to store alert messages and display them only when the current path is the admin home page. |
| Alert messages of other pages were not being displayed due to the block message being moved to `index.html`. | Move back the block message to `base.html`. |
| Materialize tooltips not working for dinamically added items. | Initialize the effect again after adding the buttons. |
| The alert message for low-stock items was also counting out-of-stock items incorrectly. | Include filter for item quantities greater than 0. |
| Page was wrongly flagged as malicious by Google | Register the page as mine on Google Search Console and send a review request |